...
In addition to the high-volume attacks from typical hosting providers, several IP addresses from non-traditional sources were observed, suggesting compromised systems within legitimate organizations.

| IP Address | ASN Organization | Location | Analysis |
| :--- | :--- | :--- | :--- |
| **200.9.154.79** | `COOPERATIVA AGUA Y SERVICIOS PUBLICOS UNQUILLO MENDIOLAZA LIMITADA` | Argentina | It is highly anomalous for a water and public services cooperative to be a source of attack traffic. This strongly indicates that a machine on their internal network has been compromised and is being used as part of a botnet. |
| **192.109.138.19** | `Municipalidad de Vicente Lopez` | Argentina | This IP is registered to a local government municipality. Observing attack traffic from a government entity suggests a significant internal security breach, with a compromised machine now participating in malicious scanning activities. |
| **129.13.189.202** | `Karlsruhe Institute of Technology` | Germany | While academic institutions can be involved in security research, unsolicited traffic targeting honeypots is more likely indicative of a compromised server or student-run project on the university's network being used for malicious scanning. |
